...
- Backend: FastAPI + OpenAI Realtime API (`agents` SDK) + Tavus CVI + SQLite + signed bearer sessions. See `apps/api/README.md`.

## Key commands
- Frontend typecheck: `cd apps/web && npx tsc --noEmit -p tsconfig.json`
...
## Env
- Frontend: `apps/web/.env.local` → `NEXT_PUBLIC_API_URL`, `NEXT_PUBLIC_WS_URL`.
- Backend: `apps/api/.env` → `OPENAI_API_KEY` for Realtime, course generation, images, and TARS visual location.
...
- Model defaults live in `apps/api/app/config.py`: `gpt-realtime-2.1`, `gpt-4o-mini-transcribe`, `gpt-5.4-mini` for course generation, and `gpt-image-2`. Prefer env overrides rather than hardcoding model names elsewhere.
- Auth/session secrets and CORS are also configured in `apps/api/.env`; copy names from `apps/api/.env.example` and never commit real credentials.

## Architecture highlights
- `apps/web/src/lib/learning/provider.tsx` (`LearnerProvider`/`useLearner`): global learner state (onboarding prefs, active course/lesson, XP/streak/confidence/badges). Persisted to `localStorage` + mirrored to backend `Profile.preferences.ctrlteach`.
- `apps/web/src/lib/courses/catalog.ts`: seed course catalog with full module/lesson structures.
- Generated courses are owner-scoped SQLite jobs managed by `apps/api/app/services/generated_courses.py`, exposed through `/api/generated-courses`, and merged into the learner catalog after completion. Course generation persists intermediate research, outline, lesson, and image stages so interrupted jobs can resume.
...
- `apps/web/src/lib/learning/labScenes.ts` + `apps/web/src/components/labs/LabCoach.tsx`: the hero pixel-precision coaching engine (Tars-inspired bezier-arc flight + live CSS-selector rect resolution). `apps/web/src/components/labs/UserStoryBuilder.tsx` is the practice surface.
...
- OpenAI voice selection is validated in `apps/api/app/agents/roleplay_agent.py` and sent in the roleplay WebSocket query. Permanent preview WAV files live in `apps/web/public/audio/roleplay-voices`; regenerate them with `apps/api/scripts/generate_roleplay_voice_previews.py` only when needed.
...
- The browser implementation in `apps/web/src/components/labs/LabCoach.tsx` owns the reference interaction: pointer overlay, `[POINT]`→selector mapping, quadratic-bezier arc flight (`animateBezierFlightArc`), streaming speech bubble, and pulsing focus.
