...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const authenticate = async (
    ...
    const loginResponse = await axios.post(`${API_URL}${endpoint}`, {
      username,
      password,
    });
    ...
    const nextUser: AuthUser = {
      ...
      isAdmin: Boolean(metadata.is_admin ?? loginResponse.data?.user?.isAdmin),
    };
    saveAuth(token, nextUser);
    setUser(nextUser);
  };
  ...
  const login = async (username: string, password: string) => {
    await authenticate(username, password, "/api/auth/login");
  };

  const adminLogin = async (username: string, password: string) => {
    await authenticate(username, password, "/api/auth/admin/login");
  };
  ...
  return (
    <AuthContext.Provider value={{ user, loading, login, adminLogin, signUp, logout, getToken }}>
      {children}
