...
from app.agents.tars_agent import (
    TARS_INSTRUCTION,
    _explicit_click_requested,
    _github_repository_private_action,
    _resolve_point_at,
)
...
class SpatialResultTests(unittest.TestCase):
    def test_github_settings_requires_scroll_before_not_found_reply(self) -> None:
        instruction = " ".join(TARS_INSTRUCTION.split())
        self.assertIn('interact_with_page(action="scroll_down", label="Developer settings")', instruction)
        self.assertIn("never say you cannot find", instruction)

    def test_github_visibility_override_uses_short_natural_lines(self) -> None:
        instruction = " ".join(TARS_INSTRUCTION.split())
        self.assertIn("owned by a deterministic server controller", instruction)
        self.assertIn("never call a tool or add a preamble", instruction)
        self.assertIn("must not jump directly to the settings url", instruction.lower())
        self.assertIn("must not click change visibility", instruction.lower())
        self.assertIn("complete github's prompts manually", instruction.lower())
        self.assertIn("do not add another explanation or ask a question", instruction)
        self.assertIn("when `browserlab` is absent", instruction.lower())
        self.assertIn("normal page companion", instruction)

    def test_coordinate_click_requires_explicit_spoken_intent(self) -> None:
        self.assertTrue(_explicit_click_requested("click the play button"))
        self.assertTrue(_explicit_click_requested("open that canvas control"))
        self.assertTrue(_explicit_click_requested("could you play the video"))
        self.assertFalse(_explicit_click_requested("point to the play button"))
        self.assertFalse(_explicit_click_requested("where is that control"))

    def test_github_private_action_requires_a_prior_request_and_public_lab_state(self) -> None:
        state = {
            "github_visibility_request_count_at_turn_start": 0,
            "github_visibility_override_granted": False,
            "companion_page_context": {
                "browserLab": {
                    "active": True,
        ...
        self.assertEqual(
            _github_repository_private_action(state)["status"],
            "repeat_required",
        )
        state["github_visibility_request_count_at_turn_start"] = 1
        action = _github_repository_private_action(state)
        self.assertEqual(action["status"], "started")
        self.assertEqual(action["action"], "github_make_private")
        self.assertTrue(state["github_visibility_override_granted"])

        state["companion_page_context"]["browserLab"]["observedState"]["repositoryVisibility"] = "private"
        self.assertEqual(
            _github_repository_private_action(state)["status"],
            "unavailable",
        )
