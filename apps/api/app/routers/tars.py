"""Global Tars assistant endpoint for browser-tab screenshots."""
...
from app.auth.extension_tokens import create_tars_extension_token
from app.config import settings
from app.services.tars_visual_locator import locate_visual_target

logger = logging.getLogger(__name__)
router = APIRouter(prefix="/api/tars", tags=["tars"])
MAX_TARS_HISTORY_TURNS = 2
MAX_TARS_IMAGE_BYTES = 6 * 1024 * 1024
MAX_TARS_DOM_CHARS = 160_000
...
class TarsRequest(BaseModel):
    ...
    image: str = Field(min_length=100, max_length=8_500_000)
    ...
    def validate_payload_size(self):
        try:
            decoded_size = len(base64.b64decode(self.image, validate=True))
        except (binascii.Error, ValueError) as exc:
            raise ValueError("image must be valid base64") from exc
        if decoded_size > MAX_TARS_IMAGE_BYTES:
            raise ValueError("image must be 6 MB or smaller")
        if len(json.dumps(self.elements, ensure_ascii=False)) > MAX_TARS_DOM_CHARS:
            raise ValueError("DOM inventory is too large")
        return self


class TarsSpeakRequest(BaseModel):
    clientRequestId: Optional[str] = Field(default=None, max_length=128)
    text: str = Field(min_length=1, max_length=4_000)


class TarsPoint(BaseModel):
    x: float
    y: float
...
class TarsResponse(BaseModel):
    ...
    point: Optional[TarsPoint] = None
    action: str = "none"
    audio_b64: Optional[str] = None
    audio_mime: Optional[str] = None


class TarsExtensionSession(BaseModel):
    access_token: str
...
def _normalize_tars_json(data: Any) -> dict[str, Any]:
...
@router.post("/extension-session", response_model=TarsExtensionSession)
async def create_extension_session(user: dict = Depends(get_current_user)):
    """Exchange an app session for a Tars-only expiring token."""
    token, expires_at = create_tars_extension_token(user)
    return TarsExtensionSession(
        access_token=token,
...
async def _speak(answer: str, request_id: str = "unknown") -> str:
    """Generate Tars's spoken audio via OpenAI TTS (fast single HTTP call).
    ...
    logger.info("tars tts start request_id=%s answer=%r", request_id, answer[:160])
    try:
        ...
        audio = response.read()
        logger.info(
            "tars tts done request_id=%s bytes=%s tts_ms=%s",
            request_id,
            len(audio),
        ...
        logger.warning("tars tts failed request_id=%s error=%s", request_id, exc)
        return ""


@router.post("")
async def ask_tars(req: TarsRequest, _user: dict = Depends(get_current_user)):
    if not settings.openai_api_key:
        return TarsResponse(answer="Tars needs an OpenAI API key to see this page.", point=None)

    request_id = req.clientRequestId or "unknown"
    total_start = time.perf_counter()
    prompt = req.prompt.strip() or "Help me with what is on this screen."
    logger.info(
        "tars request start request_id=%s conversation_id=%s prompt=%r elements=%s image=%sx%s history=%s include_audio=%s",
        request_id,
    ...
    system = (
        "You are Tars, a concise browser-tab assistant. You see a screenshot of the current app tab. "
        "Your name is Tars, but users often address you as 'chat' or 'hey chat' (e.g. 'hey chat, click that button'). "
        "When a user greets you with 'chat' or 'hey chat', treat it as addressing you and respond as Tars. "
    ...
    try:
        ...
        for turn in req.history[-MAX_TARS_HISTORY_TURNS:]:
        ...
        resp = _client().chat.completions.create(
            ...
            response_format={
                "type": "json_schema",
                "json_schema": {"name": "tars", "strict": True, "schema": SCHEMA},
            },
            temperature=0.3,
        )
        model_ms = int((time.perf_counter() - model_start) * 1000)
        raw_content = resp.choices[0].message.content or "{}"
        logger.info("tars model raw request_id=%s model_ms=%s content=%s", request_id, model_ms, raw_content[:1000])
        data = _normalize_tars_json(json.loads(raw_content))
        ...
        if not answer or answer.lower() in {"i can help with that.", "i can help with that"}:
            logger.warning("tars model returned generic/empty answer: prompt=%r data=%r", prompt, data)
            answer = "I did not catch the actual request. Say it again after the wake word."
        logger.info(
            "tars model parsed request_id=%s answer=%r target_id=%s action=%s point=%s",
            request_id,
            answer[:200],
            target_id,
            action,
        ...
        logger.info(
            "tars response done request_id=%s model_ms=%s tts_ms=%s total_ms=%s audio_b64_len=%s",
            request_id,
            model_ms,
            tts_ms,
            total_ms,
        ...
        logger.warning("tars endpoint failed: %s", exc, exc_info=True)
        return TarsResponse(answer="I had trouble seeing the page just now. Try again.", point=None)


@router.post("/speak")
async def speak_tars(req: TarsSpeakRequest, _user: dict = Depends(get_current_user)):
    request_id = req.clientRequestId or "unknown"
    text = req.text.strip()
    if not text:
